...
# ========== ФУНКЦИИ ДЛЯ ВАЙТ-ЛИСТА ==========
def add_to_whitelist(username):
    cursor.execute("INSERT OR IGNORE INTO whitelist (username, added_at) VALUES (?, ?)", 
                   (normalize_username(username), datetime.datetime.now().isoformat()))
    conn.commit()

def remove_from_whitelist(username):
    cursor.execute("DELETE FROM whitelist WHERE username = ?", (normalize_username(username),))
    conn.commit()

def is_whitelisted(username):
    cursor.execute("SELECT * FROM whitelist WHERE username = ?", (normalize_username(username),))
    return cursor.fetchone() is not None

def get_whitelist():
    cursor.execute("SELECT username, added_at FROM whitelist")
...
def set_subscription(user_id, end_date):
    ...
    if cursor.rowcount == 0:
        cursor.execute("INSERT INTO users (user_id, subscription_end) VALUES (?, ?)", (user_id, end_date))
        conn.commit()
...
def create_promocode(name, max_activations, subscription_days, expiry_date):
    cursor.execute("""
        INSERT INTO promocodes (name, max_activations, subscription_days, expiry_date, created_at)
        VALUES (?, ?, ?, ?, ?)
    """, (name, max_activations, subscription_days, expiry_date, datetime.datetime.now().isoformat()))
    conn.commit()
...
def update_promocode(promo_id, field, value):
    cursor.execute(f"UPDATE promocodes SET {field} = ? WHERE id = ?", (value, promo_id))
    conn.commit()
...
def activate_promocode(promo_id, user_id):
    ...
    cursor.execute("INSERT INTO promocode_activations (promo_id, user_id, activated_at) VALUES (?, ?, ?)",
                   (promo_id, user_id, datetime.datetime.now().isoformat()))
...
def get_promocode_activations(promo_id):
    cursor.execute("SELECT * FROM promocode_activations WHERE promo_id = ?", (promo_id,))
    return cursor.fetchall()

# ========== ФУНКЦИИ ДЛЯ ЛОГОВ РЕПОРТОВ ==========
def add_report_log(user_id, username, target, method, reports_count):
    cursor.execute("""
        INSERT INTO reports_log (user_id, username, target, method, reports_count, date)
        VALUES (?, ?, ?, ?, ?, ?)
    """, (user_id, username, target, method, reports_count, datetime.datetime.now().isoformat()))
    conn.commit()
...
async def send_subscription_notification(context, user_id, days, source="purchase"):
    ...
    msg = await send_with_photo(context, user_id, text, main_menu(user_id))
    return msg

# ========== ФИКТИВНЫЙ ОТЧЁТ О СНОСЕ ==========
...
async def send_progress_message(context, chat_id, target, reports_count, method_name):
    start_text = f"🚀 Запущен процесс отправки репортов на {target}\n📋 Метод: {method_name}\n\n🔄 Подготовка..."
    start_msg = await send_without_quote(context, chat_id, start_text)
    ...
    add_report_log(chat_id, None, target, method_name, reports_count)
...
def stars_menu():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("1 день - 75 ⭐", callback_data="pay_1")],
        [InlineKeyboardButton("3 дня - 125 ⭐", callback_data="pay_3")],
        [InlineKeyboardButton("7 дней - 200 ⭐", callback_data="pay_7")],
        [InlineKeyboardButton("30 дней - 750 ⭐", callback_data="pay_30")],
        [InlineKeyboardButton("🔙 Назад", callback_data="buy")]
    ])

def ton_menu():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("1 день - 1.5 TON", callback_data="ton_1")],
        [InlineKeyboardButton("3 дня - 2.5 TON", callback_data="ton_3")],
        [InlineKeyboardButton("7 дней - 4 TON", callback_data="ton_7")],
        [InlineKeyboardButton("30 дней - 10 TON", callback_data="ton_30")],
        [InlineKeyboardButton("🔙 Назад", callback_data="buy")]
    ])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cursor.execute("INSERT OR IGNORE INTO users (user_id, username) VALUES (?, ?)", (user.id, username))
    conn.commit()
...
async def create_promo_step4(update: Update, context: ContextTypes.DEFAULT_TYPE, text):
    """Шаг 4: Ввод названия и создание"""
    try:
...
async def create_promo_finish(update: Update, context: ContextTypes.DEFAULT_TYPE, text):
    """Финальный шаг: создание промокода"""
    name = text
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("promo_view_"):
        ...
        text = (
            ...
            f"📌 Статус: {'Активен ✅' if promo[6] == 1 else 'Неактивен ❌'}"
        )
        keyboard = [
    ...
    if data.startswith("log_user_"):
        ...
        keyboard = []
        for log in logs:
            keyboard.append([InlineKeyboardButton(f"🎯 {log[2]} ({log[3]})", callback_data=f"log_target_{log[0]}")])
    ...
    if data.startswith("log_target_"):
        ...
        cursor.execute("SELECT * FROM reports_log WHERE id = ?", (log_id,))
        log = cursor.fetchone()
        if not log:
            text = "❌ Лог не найден."
            msg = await send_with_photo(context, user.id, text, admin_main_menu())
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text.startswith("/") and "admin_action" not in context.user_data and "whitelist_action" not in context.user_data and "snos_action" not in context.user_data:
        promo = None
        cursor.execute("SELECT * FROM promocodes WHERE name = ? AND active = 1", (text,))
        ...
        if promo:
            ...
            if success:
                ...
                text = f"✅ Промокод активирован! Подписка на {days} дней оформлена 🎉"
            else:
                text = f"❌ {message}"
        else:
...
# Закомментировано для запуска через app.py на Render
