...
def supabase_retry(func, *args, **kwargs):
    max_retries = 5
...
# ========== ФУНКЦИИ ДЛЯ РАБОТЫ С БД С ПОВТОРНЫМИ ПОПЫТКАМИ ==========
...
def is_whitelisted(username):
    ...
    return supabase_retry(_check) or False
...
def activate_promocode(promo_id, user_id):
    ...
    return supabase_retry(_activate) or (False, "Ошибка активации")
...
# ========== ЗАПУСК ==========
def main():
    time.sleep(3)  # Ждём 3 секунды перед подключением
    ...
    try:
        app.bot.get_updates(offset=-1, timeout=1)
    except:
        pass
