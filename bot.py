...
def get_all_ref_links():
    try:
        res = supabase.table("ref_links").select("*").order("created_at", desc=False).execute()
        return [(r["code"], r.get("name") or "", r.get("created_at") or "", r.get("owner_id")) for r in res.data]
    except Exception as e:
        log_err(f"❌ get_all_ref_links: {e}")
        return []

def get_ref_link(code):
    try:
        res = supabase.table("ref_links").select("*").eq("code", code).execute()
        if res.data:
            r = res.data[0]
            return (r["code"], r.get("name") or "", r.get("created_at") or "", r.get("owner_id"))
    except Exception as e:
        log_err(f"❌ get_ref_link: {e}")
    return None

def get_user_ref_link(user_id):
    try:
        res = supabase.table("ref_links").select("*").eq("owner_id", user_id).execute()
        if res.data:
            r = res.data[0]
            return (r["code"], r.get("name") or "", r.get("created_at") or "", r.get("owner_id"))
    except Exception as e:
        log_err(f"❌ get_user_ref_link: {e}")
    return None

def create_ref_link(name="", owner_id=None):
    code = ''.join(random.choices(string.ascii_lowercase + string.digits, k=10))
    try:
        supabase.table("ref_links").insert({
            "code": code, "name": name, "owner_id": owner_id,
            "created_at": datetime.datetime.now().isoformat()
        }).execute()
        log_event(f"🔗 Реф-ссылка создана: {code} ({name}) owner={owner_id}")
        return code
    except Exception as e:
        log_err(f"❌ create_ref_link: {e}")
    return None

def delete_ref_link(code):
    try:
        supabase.table("ref_links").delete().eq("code", code).execute()
        supabase.table("ref_joins").delete().eq("code", code).execute()
        log_event(f"🗑 Реф-ссылка удалена: {code}")
    except Exception as e:
        log_err(f"❌ delete_ref_link: {e}")

def add_ref_join(code, user_id):
    try:
        existing = supabase.table("ref_joins").select("*").eq("code", code).eq("user_id", user_id).execute()
        if not existing.data:
            supabase.table("ref_joins").insert({
                "code": code, "user_id": user_id,
                "joined_at": datetime.datetime.now().isoformat()
            }).execute()
            log_event(f"🔗 Новый переход по {code}: юзер {user_id}")
    except Exception as e:
        log_err(f"❌ add_ref_join: {e}")

def get_ref_stats(code):
    """Возвращает (total_joins, buyers, total_stars, total_ton, list_of_users)."""
    try:
        joins_res = supabase.table("ref_joins").select("*").eq("code", code).order("joined_at", desc=False).execute()
        joins = joins_res.data
        ...
        for j in joins:
            uid = j["user_id"]
            u = get_user(uid)
            uname = u[1] if u and u[1] != "NULL" else f"id{uid}"
            bought = False
            try:
                st = supabase.table("payments_stars").select("*").eq("user_id", uid).execute()
                if st.data:
                    bought = True
                    for s in st.data:
                        try: total_stars += int(s.get("price") or 0)
                        except: pass
            except: pass
            try:
                tn = supabase.table("payments_ton").select("*").eq("user_id", uid).eq("status", "подтверждено").execute()
                if tn.data:
                    bought = True
                    for t in tn.data:
                        try: total_ton += float(t.get("ton_amount") or 0)
                        except: pass
            except: pass
            if bought: buyers += 1
            users_list.append((uid, uname, bought))
        return (total_joins, buyers, total_stars, total_ton, users_list)
    except Exception as e:
        log_err(f"❌ get_ref_stats: {e}")
        return (0, 0, 0, 0.0, [])
...
def main_menu(user_id):
    ...
    if get_user_ref_link(user_id):
        rows.append([_btn("Статистика", "my_stats", style="primary", emoji_id=CE_LOG)])
    return rows
...
def ton_menu():
    return [
        [_btn("1 день - 5 TON", "ton_1", style="danger", emoji_id=CE_TON_EMOJI)],
        [_btn("3 дня - 7.5 TON", "ton_3", style="danger", emoji_id=CE_TON_EMOJI)],
        [_btn("7 дней - 12.5 TON", "ton_7", style="danger", emoji_id=CE_TON_EMOJI)],
        [_btn("30 дней - 17.5 TON", "ton_30", style="danger", emoji_id=CE_TON_EMOJI)],
...
def admin_main_menu():
    return [
        ...
        [_btn("Создать оплату", "admin_create_payment", style="success", emoji_id=CE_BUY), _btn("Ссылки", "admin_ref", style="primary", emoji_id=CE_LINK)],
        [_btn("Назад", "back_main", style="danger", emoji_id=CE_BACK)],
    ]
...
def admin_ref_menu():
    return [
        [_btn("Создать ссылку", "admin_ref_create", style="success", emoji_id=CE_CHECK)],
        [_btn("Список", "admin_ref_list", style="primary", emoji_id=CE_LOG)],
        [_btn("Назад", "admin_back", style="primary", emoji_id=CE_BACK)],
    ]

def admin_ref_list_menu(links):
    rows = [[_btn("Назад", "admin_ref", style="primary", emoji_id=CE_BACK)]]
    for link in links:
        code = link[0]
        name = link[1]
        owner_id = link[3] if len(link) > 3 else None
        owner_display = ""
        if owner_id:
            u = get_user(owner_id)
            uname = u[1] if u and u[1] != "NULL" else f"id{owner_id}"
            owner_display = f" @{uname}"
        label = f"{name or code}{owner_display}"
        rows.append([_btn(label[:40], f"admin_ref_view_{code}", emoji_id=CE_LOG)])
    return rows

def admin_ref_view_menu(code):
    return [
        [_btn("Удалить", f"admin_ref_del_{code}", style="danger", emoji_id=CE_CROSS)],
        [_btn("Назад", "admin_ref_list", style="primary", emoji_id=CE_BACK)],
    ]
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if r.status_code == 200:
                    ...
                    log_err(f"❌ sendMessage отклонён: {r.status_code}: {r.text}")
        except Exception as e:
            log_err(f"❌ sendMessage упал: {type(e).__name__}: {e}")
...
async def schedule_fake_report(context, chat_id, target, reports_count):
    ...
    async def send_fake_report():
        ...
        log_event(f"📩 Фейк-отчёт {'✅' if success else '❌'} отправлен юзеру {chat_id}")
...
async def send_invoice(chat_id, context, days, price, payload=None):
    ...
    try:
        ...
        log_err(f"❌ Ошибка отправки счёта {chat_id}: {type(e).__name__}: {e}")
        return False
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        ...
        if arg.startswith("ref_"):
            ...
            link = get_ref_link(code)
            if link:
                add_ref_join(code, user.id)
                log_event(f"🔗 Юзер {user.id} пришёл по реф-ссылке {code}")
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_ref":
        if user.id != ADMIN_ID:
            await query.answer("⛔ Доступ запрещён.", show_alert=True); return
        msg = await send_with_photo(context, user.id, "🔗 <b>Реферальные ссылки:</b>", admin_ref_menu())
        await add_message_to_delete(context, user.id, user.id, msg)
        try: await query.message.delete()
        except: pass
        return

    if data == "admin_ref_create":
        if user.id != ADMIN_ID:
            await query.answer("⛔ Доступ запрещён.", show_alert=True); return
        context.user_data["ref_create_step"] = "waiting_name"
        msg = await send_with_photo(context, user.id, "📝 <b>Введите название для ссылки</b> (например: «для канала»):", admin_back_menu())
        await add_message_to_delete(context, user.id, user.id, msg)
        try: await query.message.delete()
        except: pass
        return
    ...
    if data == "my_stats":
        link = get_user_ref_link(user.id)
        if not link:
            await query.answer("❌ У вас нет привязанной ссылки.", show_alert=True)
            return
        code = link[0]
        total_joins, buyers, stars, ton, users_list = get_ref_stats(code)
        bot_username = (await context.bot.get_me()).username
        text = (
            f"📊 <b>Ваша статистика</b>\n\n"
            f"📝 Название: <i>{link[1] or '—'}</i>\n"
            f"🔗 <code>https://t.me/{bot_username}?start=ref_{code}</code>\n\n"
            f"👥 Переходов: <b>{total_joins}</b>\n"
            f"💰 Покупателей: <b>{buyers}</b>\n"
            f"⭐ Куплено звёзд: <b>{stars}</b>\n"
            f"💎 Куплено TON: <b>{ton:.2f}</b>"
        )
    ...
    if data == "admin_ref_list":
        if user.id != ADMIN_ID:
            await query.answer("⛔ Доступ запрещён.", show_alert=True); return
        links = get_all_ref_links()
        if not links:
            msg = await send_with_photo(context, user.id, "📋 <b>Ссылок пока нет.</b>", admin_ref_menu())
            await add_message_to_delete(context, user.id, user.id, msg)
            try: await query.message.delete()
            except: pass
            return
        msg = await send_with_photo(context, user.id, "📋 <b>Все ссылки:</b>", admin_ref_list_menu(links))
        await add_message_to_delete(context, user.id, user.id, msg)
        try: await query.message.delete()
        except: pass
        return

    if data.startswith("admin_ref_view_"):
        if user.id != ADMIN_ID:
            await query.answer("⛔ Доступ запрещён.", show_alert=True); return
        code = data.replace("admin_ref_view_", "")
        link = get_ref_link(code)
        if not link:
            await query.answer("❌ Ссылка не найдена.", show_alert=True); return
        name, created_at, owner_id = link[1], link[2], link[3]
        owner_display = "—"
        if owner_id:
            u = get_user(owner_id)
            uname = u[1] if u and u[1] != "NULL" else f"id{owner_id}"
            owner_display = f"@{uname} (ID: <code>{owner_id}</code>)"
        total_joins, buyers, stars, ton, users_list = get_ref_stats(code)
        bot_username = (await context.bot.get_me()).username
        text = (
            f"🔗 <b>Ссылка:</b> <code>{code}</code>\n"
            f"📝 Название: <i>{name or '—'}</i>\n"
            f"👤 Владелец: {owner_display}\n"
            f"📅 Создана: <i>{created_at[:16]}</i>\n\n"
            f"👥 Переходов: <b>{total_joins}</b>\n"
            f"💰 Покупателей: <b>{buyers}</b>\n"
            f"⭐ Куплено звёзд: <b>{stars}</b>\n"
            f"💎 Куплено TON: <b>{ton:.2f}</b>\n\n"
        ...
        msg = await send_with_photo(context, user.id, text, admin_ref_view_menu(code))
        await add_message_to_delete(context, user.id, user.id, msg)
        try: await query.message.delete()
        except: pass
        return

    if data.startswith("admin_ref_del_"):
        if user.id != ADMIN_ID:
            await query.answer("⛔ Доступ запрещён.", show_alert=True); return
        code = data.replace("admin_ref_del_", "")
        delete_ref_link(code)
        msg = await send_with_photo(context, user.id, "✅ <b>Ссылка удалена.</b>", admin_ref_menu())
        await add_message_to_delete(context, user.id, user.id, msg)
        try: await query.message.delete()
        except: pass
        return
    ...
    if data.startswith("ton_") and data not in ["ton_pay", "ton_cancel"] and not data.startswith("ton_check_") and not data.startswith("ton_confirm_") and not data.startswith("ton_cancel_") and not data.startswith("ton_back_"):
        ...
        prices = {"ton_1": "5", "ton_3": "7.5", "ton_7": "12.5", "ton_30": "17.5"}
        if data in days_map:
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id == ADMIN_ID and context.user_data.get("ref_create_step") == "waiting_name":
        name = text
        context.user_data["ref_create_name"] = name
        context.user_data["ref_create_step"] = "waiting_owner"
        try: await update.message.delete()
        except: pass
        await delete_previous_messages(context, user.id, user.id, keep_first=True)
        msg = await send_with_photo(context, user.id, "👤 <b>Введите ID пользователя</b>, к которому привязать ссылку (он получит кнопку «Статистика»):", admin_back_menu())
        await add_message_to_delete(context, user.id, user.id, msg)
        return

    if user.id == ADMIN_ID and context.user_data.get("ref_create_step") == "waiting_owner":
        owner_id = None
        if text.isdigit():
            owner_id = int(text)
        if not owner_id:
            await delete_previous_messages(context, user.id, user.id, keep_first=True)
            msg = await send_with_photo(context, user.id, "❌ <b>Введите числовой ID!</b>", admin_back_menu())
            await add_message_to_delete(context, user.id, user.id, msg)
            return
        owner = get_user(owner_id)
        if not owner:
            await delete_previous_messages(context, user.id, user.id, keep_first=True)
            msg = await send_with_photo(context, user.id, f"❌ <b>Пользователь с ID</b> <code>{owner_id}</code> <b>не найден в БД.</b>", admin_back_menu())
            await add_message_to_delete(context, user.id, user.id, msg)
            return
        name = context.user_data.get("ref_create_name", "")
        code = create_ref_link(name, owner_id)
        context.user_data.pop("ref_create_step", None)
        context.user_data.pop("ref_create_name", None)
        try: await update.message.delete()
        except: pass
        bot_username = (await context.bot.get_me()).username
        link = f"https://t.me/{bot_username}?start=ref_{code}"
        owner_name = owner[1] if owner[1] != "NULL" else f"id{owner_id}"
        text_msg = (
            f"✅ <b>Ссылка создана!</b>\n\n"
            f"📝 Название: <i>{name}</i>\n"
            f"👤 Владелец: @{owner_name} (ID: <code>{owner_id}</code>)\n"
            f"🔗 Ссылка:\n<code>{link}</code>\n\n"
            f"<i>У владельца теперь есть кнопка «Статистика».</i>"
        )
        await delete_previous_messages(context, user.id, user.id, keep_first=True)
        msg = await send_with_photo(context, user.id, text_msg, admin_ref_menu())
        await add_message_to_delete(context, user.id, user.id, msg)
        return
    ...
    if not text.startswith("/") and "admin_action" not in context.user_data and "whitelist_action" not in context.user_data and "snos_action" not in context.user_data and "acp_step" not in context.user_data and "broadcast_step" not in context.user_data and "ref_create_step" not in context.user_data:
...
async def successful_payment_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if payload.startswith("custom_"):
        ...
        try:
            await context.bot.send_message(chat_id=ADMIN_ID, text=f"💰 Кастомная оплата\n👤 ID: {user.id}\n💵 {total_amount}⭐")
        except: pass
        return
...
def startup_report():
    ...
    log_event(f"🔗 Реф-ссылок: {len(get_all_ref_links())}")
