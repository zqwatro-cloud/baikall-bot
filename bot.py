...
def get_report_logs_by_user(user_id):
    try:
        ...
        for i, r in enumerate(res.data, start=1):
            logs.append((i, r["user_id"], r["username"], r["target"], r["method"], r["reports_count"], r["created_at"]))
        return logs
    except Exception as e:
...
def get_report_log_by_id(log_id):
    try:
        res = supabase.table("reports_log").select("*").order("log_id", desc=False).execute()
        if 1 <= log_id <= len(res.data):
            r = res.data[log_id - 1]
            return (log_id, r["user_id"], r["username"], r["target"], r["method"], r["reports_count"], r["created_at"])
    except Exception as e:
...
def main_menu(user_id):
    rows = [
        [_btn("📤 Отправка", "send", style="primary"), _btn("👤 Профиль", "profile", style="primary")],
        [_btn("📞 Поддержка", url=SUPPORT_URL, style="danger"), _btn("📢 Наш канал", url="https://t.me/baikallShop", style="danger")],
    ]
    if user_id == ADMIN_ID:
        rows.append([_btn("🔐 Админка", "admin", style="success")])
    return rows

def profile_menu():
    return [
        [_btn("🎟 Промокод", "promo", style="danger")],
        [_btn("💳 Купить подписку", "buy", style="success")],
        [_btn("🔙 Назад", "back_main", style="primary")],
    ]

def promo_enter_menu():
    return [[_btn("🔙 Назад", "profile", style="primary")]]

def buy_menu():
    return [
        [_btn("⭐ Звёздами", "stars", style="success")],
        [_btn("💎 TON", "ton_pay", style="primary")],
        [_btn("🔙 Назад", "profile", style="primary")],
    ]

def stars_menu():
    return [
        [_btn("1 день - 500 ⭐", "pay_1", style="danger")],
        [_btn("3 дня - 1000 ⭐", "pay_3", style="danger")],
        [_btn("7 дней - 2000 ⭐", "pay_7", style="danger")],
        [_btn("30 дней - 5000 ⭐", "pay_30", style="danger")],
        [_btn("⏱ Своя подписка", "own_sub_stars", style="primary")],
        [_btn("🔙 Назад", "buy", style="primary")],
    ]

def ton_menu():
    return [
        [_btn("1 день - 1.5 TON", "ton_1", style="danger")],
        [_btn("3 дня - 2.5 TON", "ton_3", style="danger")],
        [_btn("7 дней - 4 TON", "ton_7", style="danger")],
        [_btn("30 дней - 10 TON", "ton_30", style="danger")],
        [_btn("⏱ Своя подписка", "own_sub_ton", style="primary")],
        [_btn("🔙 Назад", "buy", style="primary")],
    ]

def own_sub_menu(back_cb):
    return [
        [_btn("📩 Перейти к оформлению", url=SUPPORT_URL, style="success")],
        [_btn("🔙 Назад", back_cb, style="primary")],
    ]

def ton_confirm_menu(code, days, user_id):
    return [
        [_btn("❌ Отмена", "ton_cancel", style="danger")],
        [_btn("🔍 Проверить", f"ton_check_{code}_{user_id}", style="primary")],
    ]

def ton_admin_menu(code, days, user_id):
    return [
        [_btn("✅ Принять", f"ton_confirm_{code}", style="success")],
        [_btn("❌ Отменить", f"ton_cancel_admin_{code}", style="danger")],
    ]

def ton_cancel_confirm_menu(code, days, user_id):
    return [
        [_btn("❌ Точно отменить", f"ton_cancel_final_{code}", style="danger")],
        [_btn("🔙 Назад", f"ton_back_{code}", style="primary")],
    ]

def admin_main_menu():
    return [
        [_btn("✅ Выдать подписку", "admin_give", style="success"), _btn("❌ Забрать подписку", "admin_remove", style="danger")],
        [_btn("🎟 Создать промокод", "admin_create_promo", style="danger"), _btn("📋 Промокоды", "admin_list_promo", style="danger")],
        [_btn("📄 Выдать лог", "admin_send_log", style="primary"), _btn("⬜ Вайт-лист", "admin_whitelist", style="primary")],
        [_btn("📢 Рассылка", "admin_broadcast", style="primary"), _btn("🗄 Database", "admin_database", style="success")],
        [_btn("💳 Создать оплату", "admin_create_payment", style="success"), _btn("🔙 Назад", "back_main", style="danger")],
    ]

def admin_back_menu():
    return [[_btn("🔙 Назад", "admin_back", style="primary")]]

def admin_input_back_menu():
    return [
        [_btn("❌ Отмена", "admin_back", style="danger")],
        [_btn("🔙 Назад", "admin_back", style="primary")],
    ]

def admin_whitelist_menu():
    return [
        [_btn("➕ Добавить", "whitelist_add", style="success")],
        [_btn("🗑 Удалить", "whitelist_remove", style="danger")],
        [_btn("📋 Список", "whitelist_list", style="primary")],
        [_btn("🔙 Назад", "admin_back", style="primary")],
    ]
...
def admin_days_menu(user_id):
    return [
        ...
        [_btn("⏱ Своё время", f"admin_custom_{user_id}", style="primary")],
        [_btn("🔙 Назад", "admin_give", style="primary")],
    ]

def broadcast_preview_menu():
    return [
        [_btn("📤 Отправить", "broadcast_send", style="success")],
        [_btn("❌ Отменить", "broadcast_cancel", style="danger")],
    ]

def admin_database_menu(page, total_pages, users_on_page):
    rows = [[_btn("🔙 Назад", "admin_back", style="primary")]]
    row = []
    for u in users_on_page:
        username = u[1] if u[1] != "NULL" else f"id{u[0]}"
        row.append(_btn(f"👤 {username}"[:25], f"db_user_{u[0]}", style="danger"))
        if len(row) == 2:
            rows.append(row)
            row = []
    if row: rows.append(row)
    nav = []
    if page > 0: nav.append(_btn("⬅️ Назад", f"db_page_{page-1}", style="primary"))
    if page < total_pages - 1: nav.append(_btn("Вперёд ➡️", f"db_page_{page+1}", style="primary"))
    if nav: rows.append(nav)
    return rows

def db_user_menu(user_id, current_page, cooldown_active_for_user):
    if cooldown_active_for_user:
        cd_btn = _btn("🟢 Кулдаун: ВКЛ", f"db_toggle_cd_{user_id}", style="success")
    else:
        cd_btn = _btn("🔴 Кулдаун: ВЫКЛ", f"db_toggle_cd_{user_id}", style="danger")
    return [
        [cd_btn],
        [_btn("🔙 Назад", f"db_page_{current_page}", style="primary")],
    ]

def send_menu():
    return [
        [_btn("👤 Снос аккаунта", "snos_acc", style="danger")],
        [_btn("🤖 Снос бота", "snos_bot", style="danger")],
        [_btn("👥 Снос группы", "snos_group", style="danger")],
        [_btn("📢 Снос канала", "snos_channel", style="danger")],
        [_btn("🔙 Назад", "back_main", style="primary")],
    ]

def snos_input_menu():
    return [
        [_btn("❌ Отмена", "back_main", style="danger")],
        [_btn("🔙 Назад", "send", style="primary")],
    ]

def method_menu(action):
    return [
        [_btn("🌐 Универсальный", f"method_{action}_universal", style="primary")],
        [_btn("📨 Спам", f"method_{action}_spam")],
        [_btn("🔞 Порнография", f"method_{action}_porno")],
        [_btn("📱 Физ. номер", f"method_{action}_phone")],
        [_btn("🔙 Назад", "send", style="primary")],
    ]

def confirm_menu():
    return [
        [_btn("✅ Да", "confirm_yes", style="success")],
        [_btn("❌ Нет", "confirm_no", style="danger")],
    ]

def admin_create_payment_type_menu():
    return [
        [_btn("⭐ Stars", "acp_type_stars", style="success")],
        [_btn("💎 TON", "acp_type_ton", style="primary")],
        [_btn("🔙 Назад", "admin_back", style="primary")],
    ]
...
def admin_create_payment_term_menu():
    return [
        ...
        [_btn("⏱ Своё время", "acp_term_custom", style="primary")],
        [_btn("🔙 Назад", "admin_back", style="primary")],
    ]

def admin_create_payment_confirm_menu():
    return [
        [_btn("📤 Отправить счёт", "acp_send", style="success")],
        [_btn("❌ Отменить", "admin_back", style="danger")],
    ]
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if r.status_code == 200:
                    ...
                    log_err(f"❌ sendMessage со style отклонён: {r.status_code}: {r.text}")
        except Exception as e:
            log_err(f"❌ sendMessage со style упал: {type(e).__name__}: {e}")
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_list_promo":
        ...
        rows.append([_btn("🔙 Назад", "admin_back", style="primary")])
    ...
    if data.startswith("promo_view_"):
        ...
        rows = [
            [_btn("🗑 Удалить", f"promo_delete_{promo_id}", style="danger")],
            [_btn("📅 Продлить", f"promo_extend_{promo_id}", style="primary")],
            [_btn("➕ Добавить активации", f"promo_add_activations_{promo_id}", style="primary")],
            [_btn("⏱ Добавить подписку", f"promo_add_subscription_{promo_id}", style="primary")],
            [_btn("🔙 Назад", "admin_list_promo", style="primary")],
        ]
    ...
    if data == "admin_send_log":
        ...
        if not logs:
            ...
            return
        rows = [[_btn(f"👤 {log[1] if log[1] != 'NULL' else f'id{log[0]}'}", f"log_user_{log[0]}")] for log in logs]
        rows.append([_btn("🔙 Назад", "admin_back", style="primary")])
    ...
    if data.startswith("log_user_"):
        ...
        rows.append([_btn("🔙 Назад", "admin_send_log", style="primary")])
    ...
    if data.startswith("log_target_"):
        ...
        rows = [
            [_btn("📤 Отправить", f"log_send_{log_id}", style="success")],
            [_btn("❌ Отменить", f"log_cancel_{log_id}", style="danger")],
            [_btn("🔙 Назад", "admin_back", style="primary")],
        ]
    ...
    if data == "send":
        ...
        if not is_cooldown_off(user.id):
            ...
            if remaining_text:
                ...
                await delete_previous_messages(context, user.id, user.id, keep_first=True)
                text_cd = f"⏳ <b>Кулдаун активен</b>\n\nВы сможете отправить следующую цель через: <b>{remaining_text}</b>\n<i>(до {end_dt.strftime('%H:%M')})</i>"
                msg = await send_with_photo(context, user.id, text_cd, main_menu(user.id))
                await add_message_to_delete(context, user.id, user.id, msg)
                try: await query.message.delete()
                except: pass
                return
                ...
                if end_dt is not None:
                    ...
                    try:
                        await send_without_quote(context, user.id, "⏳ <b>Кулдаун завершен!</b>\n\n<i>Можете снова отправлять жалобы.</i>", None, parse_mode="HTML")
                    except: pass
