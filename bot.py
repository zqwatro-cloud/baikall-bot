...
def get_all_ref_links():
    try:
        ...
        return [(r["code"], r.get("name") or "", r.get("created_at") or "") for r in res.data]
    except Exception as e:
...
def get_ref_link(code):
    try:
        ...
        if res.data:
            r = res.data[0]
            return (r["code"], r.get("name") or "", r.get("created_at") or "")
    except Exception as e:
...
def create_ref_link(name=""):
    ...
    try:
        ...
        log_event(f"🔗 Реф-ссылка создана: {code} ({name})")
        return code
    except Exception as e:
...
def admin_ref_list_menu(links):
    ...
    for code, name, _ in links:
        label = f"{name} — {code}" if name else code
        rows.append([_btn(label[:30], f"admin_ref_view_{code}", emoji_id=CE_LOG)])
    return rows
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id == ADMIN_ID and context.user_data.get("ref_create_step") == "waiting_name":
        ...
        text_msg = (
            ...
            f"📝 Название: <i>{name}</i>\n"
            f"🔗 Ссылка:\n<code>{link}</code>\n\n"
            f"<i>Выдай эту ссылку человеку — он начнёт приводить людей.</i>"
        )
        await delete_previous_messages(context, user.id, user.id, keep_first=True)
