...
def add_user(user_id, username):
    try:
        ...
        if not existing:
            ...
            logging.info(f"✅ User {user_id} added to Supabase")
        else:
            logging.info(f"ℹ️ User {user_id} already exists")
    except Exception as e:
        logging.error(f"❌ add_user error: {e}")
        time.sleep(2)
        try:
            supabase.table("users").insert({"user_id": user_id, "username": normalize_username(username)}).execute()
            logging.info(f"✅ User {user_id} added on retry")
        except Exception as e2:
            logging.error(f"❌ add_user retry error: {e2}")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"✅ Start command completed for {user.id}")
...
def main():
    ...
    logger.info(f"🤖 Creating application with token: {BOT_TOKEN[:10]}...")
